...
func (s *SmartContract) CreateToken(ctx contractapi.TransactionContextInterface,
	id string, latitude float64, longitude float64, producer string, amount float64, largeCategory string, smallCategory string, timestamp int64) error {
...
// 購入する
func (s *SmartContract) BidOnToken(ctx contractapi.TransactionContextInterface, id string, newOwner string, newBidPrice float64, amount float64, timestamp int64) (string, error) {
	energy, err := s.ReadToken(ctx, id)
	if err != nil {
		return "", err
	}
	...
	// auctionStartTimeCompare = timestamp.Add(time.Minute * -time.Duration(auctionInterval))
...
func (s *SmartContract) AuctionEnd(ctx contractapi.TransactionContextInterface, id string, producer string, timestamp int64) (string, error) {
	energy, err := s.ReadToken(ctx, id)
...
func (s *SmartContract) QueryByStatus(ctx contractapi.TransactionContextInterface, status string, owner string) ([]*Energy, error) {
	...
	"use_index":["_design/indexStatusDoc","indexStatus"]}`, status)
...
func (s *SmartContract) QueryByUserAndTime(ctx contractapi.TransactionContextInterface, owner string, startTime int64) ([]*Energy, error) {
	queryString := fmt.Sprintf(`{"selector":{"DocType":"token","Owner":"%s", "Auction Start Time":{"$gte":%d}},
	"use_index":["_design/indexUserAndStartTimeDoc","indexUserAndStartTime"]}`, owner, startTime)
